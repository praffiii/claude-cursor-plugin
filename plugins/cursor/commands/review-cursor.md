...
- `/cursor:review-cursor` is native-review only. It does not support staged-only review, unstaged-only review, or extra focus text.
...
Background flow:
- Launch the review by asking Claude Code to run the `cursor:cursor-review` subagent in the background:
```typescript
Agent({
  subagent_type: "cursor:cursor-review",
