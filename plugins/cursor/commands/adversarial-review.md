...
- `/cursor:adversarial-review` uses the same review target selection as `/cursor:review-cursor`.
- It supports working-tree review, branch review, and `--base <ref>`.
- It does not support `--scope staged` or `--scope unstaged`.
- Unlike `/cursor:review-cursor`, it can still take extra focus text after the flags.
...
Background flow:
- Launch the review by asking Claude Code to run the `cursor:cursor-adversarial-review` subagent in the background:
```typescript
Agent({
  subagent_type: "cursor:cursor-adversarial-review",
