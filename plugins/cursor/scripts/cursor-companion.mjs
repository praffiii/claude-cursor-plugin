...
const MODEL_ALIASES = new Map([["spark", "composer-2-fast"]]);
...
function printUsage() {
  console.log(
    [
      ...
      "  node scripts/cursor-companion.mjs task [--background] [--write] [--resume-last|--resume|--fresh] [--model <model|spark>] [--effort <none|minimal|low|medium|high|xhigh>] [--sandbox <read-only|workspace-write|danger-full-access>] [prompt]",
...
function normalizeSandboxMode(sandbox) {
  ...
  const normalized = String(sandbox).trim();
  if (!normalized) {
    return null;
  }
...
async function buildSetupReport(cwd, actionsTaken = []) {
  ...
  if (!cursorStatus.available) {
    nextSteps.push("Install cursor-agent with `curl https://cursor.com/install -fsS | bash`.");
  }
  if (cursorStatus.available && !authStatus.loggedIn && authStatus.requiresCursorAuth) {
    nextSteps.push("Run `cursor-agent login`.");
    nextSteps.push("Set CURSOR_API_KEY if you prefer API-key auth.");
  }
...
function buildStandardReviewPrompt(context) {
  const template = loadPromptTemplate(ROOT_DIR, "standard-review");
...
function ensureCursorAvailable(cwd) {
  ...
  if (!availability.available) {
    throw new Error("cursor-agent CLI is not installed. Install it with `curl https://cursor.com/install -fsS | bash`, then rerun `/cursor:setup`.");
  }
}

function buildStandardReviewTarget(target) {
...
async function executeReviewRun(request) {
  ...
  if (reviewName === "Review") {
    ...
    const prompt = `${buildStandardReviewPrompt(context)}\n\nReturn only valid JSON matching this schema:\n${JSON.stringify(readOutputSchema(REVIEW_SCHEMA), null, 2)}`;
...
async function executeTaskRun(request) {
  ...
  if (request.resumeLast) {
    ...
    if (!latestThread) {
      throw new Error("No previous Cursor task thread was found for this repository.");
    }
    resumeChatId = latestThread.id;
  }
  ...
  const result = await runCursorTurn(workspaceRoot, {
    resumeChatId,
    prompt: request.prompt,
    defaultPrompt: resumeChatId ? DEFAULT_CONTINUE_PROMPT : "",
    model: request.model,
    sandbox: request.sandbox ?? (request.write ? "danger-full-access" : "read-only"),
...
async function handleCancel(argv) {
  ...
  const interrupt = await interruptCursorTurn(cwd, { threadId, turnId });
  if (interrupt.attempted) {
    appendLogLine(
      job.logFile,
      interrupt.interrupted
        ? `Requested Cursor turn interrupt for ${turnId} on ${threadId}.`
        : `Cursor turn interrupt failed${interrupt.detail ? `: ${interrupt.detail}` : "."}`
    );
  }
