...
description: Internal guidance for composing Cursor and GPT-5.4 prompts for coding, review, diagnosis, and research tasks inside the Cursor Claude Code plugin
user-invocable: false
---

# GPT-5.4 Prompting

Use this skill when `cursor:cursor-rescue` needs to ask Cursor or another GPT-5.4-based workflow for help.

Prompt Cursor like an operator, not a collaborator. Keep prompts compact and block-structured with XML tags. State the task, the output contract, the follow-through defaults, and the small set of extra constraints that matter.

Core rules:
- Prefer one clear task per Cursor run. Split unrelated asks into separate runs.
- Tell Cursor what done looks like. Do not assume it will infer the desired end state.
...
- `<default_follow_through_policy>`: what Cursor should do by default instead of asking routine questions.
...
- Write-capable tasks: add `action_safety` so Cursor stays narrow and avoids unrelated refactors.
...
- Use `task --resume-last` for follow-up instructions on the same Cursor thread. Send only the delta instruction instead of restating the whole prompt unless the direction changed materially.
...
- Ask Cursor for brief, outcome-based progress updates only when the task is long-running or tool-heavy.
...
3. Decide whether Cursor should keep going by default or stop for missing high-risk details.
...
Concrete end-to-end templates live in [references/cursor-prompt-recipes.md](references/cursor-prompt-recipes.md).
Common failure modes to avoid live in [references/cursor-prompt-antipatterns.md](references/cursor-prompt-antipatterns.md).
