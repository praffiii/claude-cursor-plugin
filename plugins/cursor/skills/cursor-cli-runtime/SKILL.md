...
- Map `spark` to `--model composer-2-fast`.
...
- Never call `task --background` from `cursor:cursor-rescue`; Cursor task execution must stay attached to the rescue subagent so Claude Code can keep the visible running task until Cursor completes.
- If the forwarded request includes `--model`, normalize `spark` to `composer-2-fast` and pass it through to `task`.
...
- Write-capable Cursor work defaults to `danger-full-access` in this fork unless the user explicitly passes `--sandbox`.
